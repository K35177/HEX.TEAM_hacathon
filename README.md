...
Текущая версия 0.7.0 — **лабораторный acoustic file transfer prototype**. Она умеет
...
- профили `turbo`, `wideband`, `fast`, `balanced` и `robust`, а также скрытый
  `turbo-v1` для чтения WAV версии 0.6.1;
...
|---|---:|---:|---:|---|
| `turbo` | 4-FSK + RS FEC | 1600 бит/с | 4096 B | рекомендуемый режим в полосе 1,2–6,0 кГц |
...
`turbo-v1` не предлагается в UI для новых передач: он нужен только для
автоматической попытки чтения старых 16-FSK записей без FEC. Повреждённую запись
старого формата восстановить невозможно, если в ней уже ошибочно приняты символы.
