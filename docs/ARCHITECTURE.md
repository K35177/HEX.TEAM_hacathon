...
Frame v1 без FEC продолжает читаться через legacy path. Приёмник автоматически пробует
`turbo-1600` для frame v2 и WAV 0.7.0, а также `turbo-v1` для 16-FSK WAV 0.6.1.
