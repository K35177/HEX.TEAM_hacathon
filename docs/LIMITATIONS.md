...
Этот файл описывает ограничения текущей версии 0.6.0. Подробные причины,
...
- Скорость до заголовков: 3200 бит/с (`turbo`), около 600 бит/с (`fast`),
  400 бит/с (`balanced`) и 66 бит/с (`robust`). `turbo` занимает полосу примерно
  1,5–15 кГц и требует широкополосных динамика и микрофона. Effective goodput
  ниже и зависит от размера файла и completion rate.
