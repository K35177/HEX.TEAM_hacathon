...
Этот файл описывает ограничения текущей версии 0.7.0. Подробные причины,
...
- Скорость PHY до FEC/заголовков: 3200 бит/с (`wideband`), 1600 бит/с (`turbo`), около
  600 бит/с (`fast`), 400 бит/с (`balanced`) и 66 бит/с (`robust`). `turbo`
  занимает 1,2–6,0 кГц. `wideband` доходит до 15 кГц и на микрофонах с
