...
Этот файл описывает ограничения текущей версии 0.5.0. Подробные причины,
...
- Live path не потоковый: `send` сначала создаёт полный WAV, `receive` сначала
  записывает фиксированный интервал.
- Автоматической остановки после конца кадра нет.
...
- Скорость до заголовков: около 600 бит/с (`fast`), 400 бит/с (`balanced`) и
  66 бит/с (`robust`). Effective goodput ниже.
...
- UI требует Python 3.10+ с Tkinter и пока не упакован в самостоятельный installer.
