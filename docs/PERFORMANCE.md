...
Дата измерений: 25 июля 2026 года. Версия: 0.7.0.
...
Основной `turbo` использует 4-FSK: 800 символов/с × 2 бита = 1600 бит/с в
полосе 1,2–6,0 кГц. Разнос несущих 1600 Гц устраняет плотную сетку, на которой
реальный тракт путал основные частоты и гармоники. Frame v2 добавляет
...
| `turbo` | 123,09 s | 1169,9 bit/s | 73,1% | 2,69× |
...
|---|---:|---:|---:|---:|
| `turbo` | 3,24 s | 632,1 bit/s | 39,5% | 32,8 ms |
...
clipping. На текущей версии все пять пользовательских профилей и совместимый
`turbo-v1` восстанавливают 6/6 payload в
