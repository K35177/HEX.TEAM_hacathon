...
Дата измерений: 25 июля 2026 года. Версия: 0.6.0.
...
## Скоростной профиль `turbo`

Добавлен 16-FSK профиль: 800 символов/с × 4 бита = 3200 бит/с. Он занимает
полосу примерно 1,5–15 кГц и сохраняет существующие framing и transfer protocol.
...
| `turbo` | 45,63 s | 3156,0 bit/s | 98,6% | 5,38× |

Дополнительный sweep `turbo` на payload 2048 B дал 60/60 успешных циклов: по
20 шумовых seed при SNR 24, 18 и 12 dB с clock drift 2000 ppm. Вариант
4800 бит/с был отклонён: при 12 dB он дал только 12/20 успешных циклов.
...
|---|---:|---:|---:|---:|
| `turbo` | 1,08 s | 1900,7 bit/s | 59,4% | 31,2 ms |
...
clipping. На текущей версии все четыре профиля восстановили 6/6 payload.
