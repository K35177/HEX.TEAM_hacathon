...
из командной строки его можно уменьшить через `--max-seconds`. Профиль задаётся
на передатчике; выбор на приёмнике определяет только первую попытку, после чего
приложение автоматически проверяет остальные режимы.
...
Для больших файлов сначала попробуйте `turbo`: он использует разнесённый 4-FSK,
RS(255,191) и interleaving в полосе 1,2–6,0 кГц. Для контрольного файла
`Duck.wav` (11 628 байт) оценка составляет около 1 мин 20 с вместо прежних
четырёх минут, при этом ошибки канала теперь исправляются. Профиль
`wideband` достигает 3200 бит/с и ускоряет эфир примерно в 5,4 раза, но использует
частоты до 15 кГц: включайте его только после успешной калибровки на конкретной
паре устройств. Если `turbo` не проходит, вернитесь к `fast` или `balanced`.
